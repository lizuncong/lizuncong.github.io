...
- 0~3：4个字节，记录核心目录文件头标识：0x02014b50，用于解压时候，查找判断是否是中央目录的开始位置 
- 4~5：2个字节，记录压缩所用的版本，同数据区本地文件头的解压所需版本，apk设置20
...
- 32~33：2个字节，记录文件注释的长度 
- 34~35：2个字节，记录文件开始位置的磁盘编号，一般传0即可 
- 内部文件属性、外部文件属性，一般也是传0即可 
