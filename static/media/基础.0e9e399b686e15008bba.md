...
这里我们调用了四次`crypto.pbkdf2`函数，这四个函数会分配到四个线程当中执行，由于线程池里面刚好有4个线程，于是这四个就绪的线程会被操作系统调度执行。又由于有4个内核轮流执行这四个线程，平均每个内核执行1个线程，平均耗时为1个PBKDF2函数的执行时间，也就是560毫秒。因此可以看到四个回调函数几乎同时执行完成，耗时差不多。
